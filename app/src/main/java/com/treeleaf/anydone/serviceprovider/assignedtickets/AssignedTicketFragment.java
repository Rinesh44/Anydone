...
import com.treeleaf.anydone.serviceprovider.assignedtickets.closedresolvedtickets.ClosedTicketsFragment;
import com.treeleaf.anydone.serviceprovider.assignedtickets.closedresolvedtickets.OnClosedTicketsListener;
import com.treeleaf.anydone.serviceprovider.assignedtickets.opentickets.OnOpenTicketsListener;
import com.treeleaf.anydone.serviceprovider.assignedtickets.opentickets.OpenTicketsFragment;
...
public class AssignedTicketFragment extends BaseFragment<AssignedTicketPresenterImpl> implements
        AssignedTicketContract.AssignedTicketsView, OnSwipeListener, OnOpenTicketsListener,
        OnClosedTicketsListener {

    private static final String TAG = "AssignedTicketFragment";
    @BindView(R.id.tabs)
    TabLayout mTabs;
    ...
    private OpenTicketListListener openTicketListListener;
    private ClosedTicketsListListener closedTicketListListener;
    private List<Tickets> openTickets;
    ...
    protected int getLayout() {
        return R.layout.fragment_assigned_tickets;
    }

    public static AssignedTicketFragment newInstance(String param1, String param2) {
        AssignedTicketFragment fragment = new AssignedTicketFragment();
  ...
  /*      serviceRequests = ServiceRequestRepo
    ...
    private void createFilterBottomSheet() {
        ...
        tvReset.setOnClickListener(v -> {
            ...
            presenter.getAssignedTickets(true);
        });
...
//        toggleBottomSheet();
    ...
    private void setupViewPager(ViewPager viewPager) {
        ...
        viewPagerAdapter.addFragment(new OpenTicketsFragment(), "Open");
    ...
    public void onSwipeRefresh() {
        ...
        presenter.getAssignedTickets(false);
    }

    @Override
    public void onOpenTicketsSeparated(List<Tickets> openTicketList) {
        GlobalUtils.showLog(TAG, "open tickets count: " + openTicketList.size());
        openTickets = openTicketList;
        if (openTicketListListener != null)
            openTicketListListener.showOpenTickets(openTickets);
        else GlobalUtils.showLog(TAG, "open ticket listener is null");
    }

    @Override
    public void getAssignedTicketsSuccess() {

    }

    @Override
    public void getAssignedTicketsFail(String msg) {
    ...
    public void onClosedTicketsSeparated(List<Tickets> closedTicketList) {
        ...
        if (closedTicketListListener != null)
            closedTicketListListener.showClosedTickets(closedTickets);
    ...
    @Override
    public void onOpenTicketsCreated() {
        GlobalUtils.showLog(TAG, "on open ticket fragment created called");
        if (openTicketListListener != null)
            openTicketListListener.showOpenTickets(openTickets);
        else GlobalUtils.showLog(TAG, "open ticket listener is null");
    }
    ...
    public interface OpenTicketListListener {
        void showOpenTickets(List<Tickets> openTicketList);
    }

    public void setOpenTicketListListener(OpenTicketListListener listener) {
        openTicketListListener = listener;
    }

    public interface ClosedTicketsListListener {
        void showClosedTickets(List<Tickets> closedTicketList);
    }

    public void setClosedTicketListListener(ClosedTicketsListListener listener) {
